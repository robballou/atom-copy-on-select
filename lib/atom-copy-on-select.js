...
var atomCopyOnSelect = {
  ...
  selectionEnded: function() {
    ...
    try {
      ...
      if (!range.isEmpty()) {
        // do not copy if the editor has multiple cursors and the option isn't
        // set to true
        if (editor.hasMultipleCursors() && !atom.config.get('atom-copy-on-select.copyOnMultipleSelections')) {
          return;
        }
